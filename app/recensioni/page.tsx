...
export default function RecensioniPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setStatus('sending')
    try {
      // Qui va la logica di invio mail reale del tuo sito
      ...
      <section className="py-16 lg:py-24 bg-background">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-20">
            {reviews.map((review) => (
              <div key={review.id} className="relative p-6 rounded-2xl bg-card border border-border/50 shadow-sm">
            ...
            <div className="bg-card rounded-3xl border-2 border-azure/10 p-8 md:p-10 shadow-2xl">
              {status === 'success' ? (
                <div className="text-center py-10">
                  ...
                  <h2 className="text-2xl font-bold mb-2">Recensione Inviata!</h2>
                  <p className="text-muted-foreground">Grazie, la pubblicheremo a breve.</p>
                  <button onClick={() => setStatus('idle')} className="mt-4 text-azure underline">Scrivine un&apos;altra</button>
                </div>
              ) : (
                <form onSubmit={handleSubmit} className="space-y-5">
                  <h2 className="text-2xl font-bold mb-6 flex items-center gap-2"><MessageSquare className="text-azure" /> Lascia il tuo Feedback</h2>
                  <div>
                    <label className="block text-sm font-semibold mb-2">Nome e Cognome *</label>
                    <input required type="text" value={nome} onChange={(e) => setNome(e.target.value)} className="w-full px-4 py-3 rounded-xl border border-border bg-background" placeholder="Mario Rossi" />
                  </div>
                  <div>
                    <label className="block text-sm font-semibold mb-2">Valutazione *</label>
                    <div className="flex gap-2">
                      {[1, 2, 3, 4, 5].map((s) => (
                        <button key={s} type="button" onClick={() => setRating(s)} onMouseEnter={() => setHover(s)} onMouseLeave={() => setHover(0)}>
                          <Star className={`h-8 w-8 ${(hover || rating) >= s ? 'fill-azure text-azure' : 'text-muted/20'}`} />
                        </button>
                      ))}
                    </div>
                  </div>
                  <div>
                    <label className="block text-sm font-semibold mb-2">Messaggio (max 200) *</label>
                    <textarea required maxLength={200} value={messaggio} onChange={(e) => setMessaggio(e.target.value)} rows={4} className="w-full px-4 py-3 rounded-xl border border-border bg-background resize-none" placeholder="La tua esperienza..." />
                  </div>
                  <button type="submit" disabled={!rating || !nome || !messaggio || status === 'sending'} className="w-full py-4 bg-azure text-white font-bold rounded-xl shadow-lg shadow-azure/20 disabled:opacity-50">
